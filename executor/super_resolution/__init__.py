...
class DiffBIR(Tool):
    ...
    def _get_cmd_opts(self) -> list[str]:
        return [
            ...
            "--sampler", "spaced",
            "--version", "v2",
            "--steps", "5",# "--steps", "50", #源代码太慢了，先注释掉，后续再调整参数让它跑得快一点
            "--upscale", "4",
            "--cfg_scale", "4",
            "--captioner", "none",
            "--output", self.output_dir,
...
class OSEDiff(Tool):
    ...
    def _get_cmd_opts(self) -> list[str]:
        return [
            ...
            "--pretrained_model_name_or_path", "Manojb/stable-diffusion-2-1-base", #改动
...
class PISASR(Tool):
    ...
    def _get_cmd_opts(self) -> list[str]:
        return [
            ...
            "--pretrained_model_path", "Manojb/stable-diffusion-2-1-base",
            "--vae_decoder_tiled_size", "170",
            "--default"
        ]
...
subtask = 'super_resolution'
sr_toolbox = [
    DiffBIR(), #成功，但是时间太长两次都崩掉了
    XRestormer(subtask=subtask),
    SwinIR(subtask=subtask, pretrained_on='gan'),
    SwinIR(subtask=subtask, pretrained_on='psnr'),
    HATPSNR(),
    HATGAN(), # 以上均成功
    OSEDiff(), # 工具失败 404问题——>成功->更换模型库名称，需要260s，4分多钟
    PISASR(), # 工具失败 ModuleNotFoundError: No module named 'src'——>成功->更换模型库名称，需要30s，半分钟
    SwinFIR(), # 成功
    NAFNet(subtask=subtask), # 成功
    HMA(), # 成功
