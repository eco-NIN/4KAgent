...
class The4KAgent:
    ...
    def propose(self) -> None:
        ...
        if hasattr(self, "perception_agent"): del self.perception_agent
    ...
    def evaluate_degradation_by_gpt4v(self) -> list[tuple[Degradation, Level]]:
        def check_evaluation(evaluation: object):
            ...
            # 👇 --- 主要修改点开始 --- 👇
            # 将原本严格的“集合完全相等(==)”校验注释掉或删除。
            # 因为大语言模型的输出具有随机性，强制要求集合完全一致容易导致无限重试死循环。
        ...
        # 👇👇👇 🚀【新增创新点 5：基于传统 CV 算子的双重混合感知校验 (降级校准版)】 👇👇👇
        img_for_cv = cv2.imread(self.cur_node["img_path"])
        if img_for_cv is not None:
            gray_img = cv2.cvtColor(img_for_cv, cv2.COLOR_BGR2GRAY)
            
            # 核心神操作：将元组列表转成字典，方便根据键名直接读取和修改严重程度 (severity)
            eval_dict = {deg: sev for deg, sev in evaluation}
            
            # 定义哪些属于“被大模型诊断为严重”的级别
            severe_levels = ["medium", "high", "very high"]
            
            # 1. 防模糊误判：只有当大模型说“有模糊”时，我们才去用算子“打假”
            if eval_dict.get("defocus blur", "very low") in severe_levels or eval_dict.get("motion blur", "very low") in severe_levels:
                laplacian_var = cv2.Laplacian(gray_img, cv2.CV_64F).var()
                self.workflow_logger.info(f"【混合感知校验】检测到 VLM 报告了模糊，启用算子复核... 拉普拉斯方差: {laplacian_var:.2f}")
                
                # 如果方差大于阈值（比如 300，说明边缘其实很锐利）
                if laplacian_var > 300.0:
                    self.workflow_logger.info("【校验驳回】图像边缘纹理锐利，推翻 GPT-4V 的模糊诊断，强制降级为 very low！")
                    # 不删除，而是将严重程度降级，完美维持原有的数据结构
                    if "defocus blur" in eval_dict: eval_dict["defocus blur"] = "very low"
                    if "motion blur" in eval_dict: eval_dict["motion blur"] = "very low"

            # 2. 防亮度误判：只有当大模型说“画面暗”时，我们才去复核
            if eval_dict.get("dark", "very low") in severe_levels:
                mean_brightness = gray_img.mean()
                self.workflow_logger.info(f"【混合感知校验】检测到 VLM 报告了暗光，启用算子复核... 平均亮度: {mean_brightness:.2f}")
                
                if mean_brightness > 80.0:
                    self.workflow_logger.info("【校验驳回】图像曝光正常，推翻 GPT-4V 的暗光诊断，强制降级为 very low！")
                    if "dark" in eval_dict: eval_dict["dark"] = "very low"
                    
            # 再将字典完美还原回大模型原本输出的元组列表格式
            evaluation = [(deg, sev) for deg, sev in eval_dict.items()]
        # 👆👆👆 ---------------- 修改结束 ---------------- 👆👆👆
    ...
    def get_image_description(self) -> str:
        ...
        if self.evaluate_degradation_by == "gpt4v":
            # 如果是 API 模式，直接调用 GPT 实例
            output = self.gpt4(prompt=prompt, img_path=Path(self.cur_node["img_path"]))
        else:
            # 如果是本地模型模式
            perception_inputs = self.perception_agent.prepare_inputs(image_path=[self.cur_node["img_path"]], 
                                                            prompts=[prompt])
    ...
    def evaluate_degradation_by_vlmagent(self) -> list[Degradation]:
        ...
        if not hasattr(self, "perception_agent"): return self.gpt4(prompt=prompt, img_path=Path(self.cur_node["img_path"]))
    ...
    def tool_selection(self, subtask: Subtask, toolbox: list[Tool], max_side: int) -> list[Tool]:
        ...
        # 👇 --- 创新点1 & 3：基于物理约束与语义先验的联合剪枝路由机制 --- 👇
        allow_heavy_model = True
        
        # 1. 物理维度约束（尺寸感知）：如果长边大于 1024，极易OOM，强制拦截大模型
        if max_side >= 1024:
            allow_heavy_model = False
            self.workflow_logger.info(f"【尺寸感知路由】触发：当前图片长边({max_side}) >= 1024，切换为极速模式。")

        # 2. 语义维度约束（退化感知）：轻量级的高频恢复任务，无需生成式模型，防幻觉
        if subtask in ["jpeg compression artifact removal", "denoising", "brightening"]:
            allow_heavy_model = False
            self.workflow_logger.info(f"【语义感知路由】触发：当前任务为轻量级({subtask})，拦截生成式大模型。")

        # 执行统一剪枝操作：不仅踢出 diffbir，而是剔除所有高耗时、大显存的模型
        if not allow_heavy_model:
            # 建立“重度模型黑名单” (统一用小写字母)
            # heavy_models = ['diffbir', 'osediff', 'DiffPlugin', 'MAXIM', 'Restormer']
            heavy_models = ['diffbir', 'osediff', 'diffplugin', 'maxim', 'restormer', 'xrestormer']
            
            # 使用 .lower() 强制转为小写比对，杜绝任何大小写漏网之鱼！
            updated_toolbox = [tool for tool in updated_toolbox if tool.tool_name.lower() not in heavy_models]
            
            self.workflow_logger.info(f"【强剪枝执行】已拦截重度模型，当前剩余轻量候选数: {len(updated_toolbox)}")
        # 👆👆👆 ---------------- 修改结束 ---------------- 👆👆👆
...
#zengjia
            # if not output_files:
            #     self.workflow_logger.warning(f"Skipping {tool.tool_name} as it produced no output.")
            #     continue
            # output_path = output_files[0]

            # # --- 新增判断逻辑：防范空目录 ---
            # output_files = sorted_glob(output_dir)
            # if not output_files:
            #     self.workflow_logger.warning(f"Skipping {tool.tool_name} as it produced no output.")
            #     continue # 直接跳到下一个 tool 的循环
            # output_path = output_files[0]
            # # -----------------------------
...
#zengjia
    ...
    def evaluate_tool_result_onetime(self, candidates: list[Path], subtask: str = "") -> tuple[Path, float]:
        if not candidates:
